...
MOUNTFLAGS := -v $(CURDIR)/data/git:/home/git -v $(CURDIR)/data/ssh:/etc/ssh -v $(CURDIR)/data/web:/var/www
...
PORTFLAGS  := -p 64801:80 -p 64822:22
...
test :
	docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) --entrypoint sh $(IMAGETAG) -ec 'git version'
