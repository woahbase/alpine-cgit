...
# runtime flags
MOUNTFLAGS := \
	-v $(CURDIR)/data/git:/home/git \
	-v $(CURDIR)/data/ssh:/etc/ssh \
	-v $(CURDIR)/data/web:/var/www \
	#
PORTFLAGS  := -p 64801:80 -p 64822:22# --net=host # so other local containers can find it without explicit linking, needs firewall cleared
PUID       := $(shell id -u)
...
bare : ## create a bare repository
	docker exec -u git -it docker_$(CNTNAME) /scripts/bareinit

mirror : ## mirror a repository
	docker exec -u git -it docker_$(CNTNAME) /scripts/mirror

sync : ## sync mirrored repositories
	docker exec -u git -it docker_$(CNTNAME) /scripts/sync
